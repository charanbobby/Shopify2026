...
| Zowie Chatbot | Storefront-Widget | **Yes** | TBD | Medium | Medium | Storefront chat widget (replaces direct Zendesk storefront integration); connects to Zendesk on the backend for ticketing. Manual embed in both; headed simpler via app blocks. [[Ref](https://shopify.dev/docs/apps/build/online-store/theme-app-extensions)] |
...
| Dscopify (DSCO / Retail Channels) | Backend-API | **No** - not MVP; post-launch | No direct native equivalent | Low | Low | **Not MVP.** Dscopify is a DSCO integration for connecting with retailers, marketplaces, and dropshipping partners via EDI/API - not a wholesale B2B tool. Backend-only; architecture-agnostic. Evaluate post-launch based on retail channel needs. |
