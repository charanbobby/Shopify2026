...
| Headless build exceeds budget | High | High | Agency: get fixed-fee SOW + scope freeze date. Self-build: lower cash outlay but increases team bandwidth risk. See [TCO](decision-framework.md#4-total-cost-of-ownership) for estimates. | TBD |
...
| Shared cart middleware complexity underestimated (headless) | **High** | High | Custom middleware required: cart ID persistence via Storefront API + unified sessions via Multipass. Not a built-in feature. Dedicated technical spike required before Apr 1. [[Ref](https://shopify.dev/docs/storefronts/headless/hydrogen/migrate)] | TBD |
...
| Headless build cost estimates are validated against chosen engagement model (self-build vs agency) | SOW from selected agency or internal estimate | TBD |
